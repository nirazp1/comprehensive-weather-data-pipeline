...
python3 generate_big_data_cli.py --cities 10000 --days 365 --output-dir ./my_big_data
```
...
See `about/DOCKER_KAFKA_SETUP.md` for detailed instructions.
...
├── fetch_openweather.py           # Basic synchronous fetcher
├── fetch_many.py                  # Async fetcher for multiple cities
├── simple_end_to_end.py           # Complete pipeline (local storage)
├── simple_end_to_end_kafka.py     # Pipeline with Kafka integration
├── streamlit_app.py               # 🌐 Web interface (Streamlit)
├── generate_big_data_cli.py       # 🚀 CLI tool for massive data generation
├── data_generator.py              # Big data generator (billions of records)
├── utils.py                       # Retries, validation, utilities
├── storage.py                     # Storage backends (S3, local)
├── kafka_producer_example.py      # Kafka producer
├── kafka_consumer.py              # Kafka consumer
├── test_kafka.py                  # Kafka setup test script
├── playwright_example.py          # Web scraping example
├── docker-compose.yml             # Docker Compose configuration
├── Dockerfile                     # Docker image definition
├── start_docker.sh                # Quick Docker start script
├── run_streamlit.sh               # Quick Streamlit start script
├── requirements.txt               # Python dependencies
├── .env.example                   # Environment variables template
├── README.md                      # This file
├── RESEARCH_PAPER.tex             # LaTeX research paper
├── STREAMLIT_GUIDE.md             # Streamlit app guide
├── about/                         # Documentation folder
│   ├── README.md                 # Documentation index
│   ├── QUICKSTART.md             # Quick start guide
│   ├── AWS_SETUP.md              # AWS configuration guide
│   ├── DOCKER_KAFKA_SETUP.md     # Docker & Kafka setup guide
...
python3 generate_big_data_cli.py --cities 10000 --days 365 --frequency hourly

# Generate 1 billion+ records (massive scale)
python3 generate_big_data_cli.py --cities 20000 --days 1825 --frequency hourly

# Generate with custom output directory
python3 generate_big_data_cli.py --cities 5000 --days 730 --output-dir ./my_big_data
```
...
data = fetch("Kathmandu")
save_raw(data, "Kathmandu")
```
...
```python
import asyncio
from fetch_many import main

# Fetch multiple cities concurrently
asyncio.run(main(cities=["Kathmandu", "Pokhara", "Lalitpur"], max_concurrent=5))
```

### End-to-End Pipeline

```python
from simple_end_to_end import run
...
```python
from normalize import normalize_openweather, get_normalizer
...
```python
from kafka_producer_example import WeatherKafkaProducer
...
```python
# airflow_dag_example.py (conceptual)
from airflow import DAG
from airflow.operators.python import PythonOperator

dag = DAG('weather_ingestion', schedule_interval='@hourly')
...
fetch_task >> normalize_task >> store_task
```
