"""End-to-end pipeline with Kafka integration: fetch → normalize → Kafka → Parquet."""
import requests
import pandas as pd
import time
...
from src.normalize import normalize_openweather
from src.utils import validate_weather_record, logger
...
def run(city: str = "Kathmandu", validate: bool = True, use_kafka: bool = True) -> Optional[Path]:
    """
    Run end-to-end pipeline for a single city with Kafka integration.
    
    Args:
        city: City name to process
        validate: Whether to validate the normalized record
        use_kafka: Whether to send data to Kafka
    ...
    try:
        ...
        # Step 4: Send to Kafka (if enabled)
        if use_kafka:
            try:
                producer = WeatherKafkaProducer()
                producer.produce(normalized, key=city)
                producer.flush()
                logger.info(f"✓ Sent {city} data to Kafka")
            except Exception as e:
                logger.warning(f"Could not send to Kafka (continuing anyway): {e}")
...
def main():
    """Main entry point for end-to-end pipeline with Kafka."""
    ...
    try:
        ...
        logger.warning(f"Kafka not available (will continue without it): {e}")
        use_kafka = False
    
    results = []
    for city in cities:
        result = run(city, use_kafka=use_kafka)
        if result:
    ...
    if use_kafka:
        print(f"✓ Data sent to Kafka topic: {settings.kafka_topic}")
